...
int open_input_file(const char *input_data_source) {
    ...
    ret = avformat_open_input(&ic, input_data_source, format, &options);
    if (ret < 0) {
        av_err2str(ret);
        return ret;
    }
    ...
    for (int i = 0; i < nb_output_streams; ++i) {
        ...
        switch (ost->enc_ctx->codec_type) {
            case AVMEDIA_TYPE_VIDEO:
                ...
                ost->enc_ctx->sample_aspect_ratio = ost->filter->inputs[0]->sample_aspect_ratio;
                break;
...
//    av_dict_set(&encoder_opts, "threads", "auto", 0);
    ...
    av_dump_format(output_file->oc, 0, output_file->oc->filename, 1);
    //写文件头
...
int open_output_file(const char *output_data_source, MediaSource *mediaSource, int new_width, int new_height) {
    ...
    ret = avformat_alloc_output_context2(&oc, NULL, "flv", output_data_source);
    if (ret < 0) {
        av_err2str(ret);
        return ret;
    }
